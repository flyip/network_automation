...
class Device(db.Model):
    ...
    def get_url(self):
        return url_for('get_device', hostname=self.hostname, _external=True)
...
def get_device(hostname):
    return jsonify(Device.query.filter_by(hostname=hostname).first_or_404().export_data())

@app.route('/devices/', methods=['POST'])
def new_device():
    device = Device()
    data = request.json
    pre_hostname = data['hostname']
    if device.query.filter_by(hostname=pre_hostname).first():
        return jsonify({}), 400
    else:
